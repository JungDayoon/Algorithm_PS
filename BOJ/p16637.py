...
def solution(prev, prev_num, prev_oper):
    global maxSum
    ...
    for i in range(prev, len(Calcul)):
        ...
        prev_num = calculate(prev_num, Calcul[i], prev_oper)
        if i == len(Calcul)-2:
            prev_oper = calculStr[i * 2 + 3]
            prev_num = calculate(prev_num, calculStr[N-1], prev_oper)
            maxSum = max(maxSum, int(prev_num))
            prev_num = calculate(tmp_num, calculStr[i * 2], tmp_oper)
            prev_oper = calculStr[i*2 + 1]
            continue
        if i == len(Calcul)-1:
            maxSum = max(maxSum, int(prev_num))
            prev_num = calculate(tmp_num, calculStr[i * 2], tmp_oper)
            ...
            continue
        prev_oper = calculStr[i*2+3]
        solution(i+2, prev_num, prev_oper)
        prev_num = calculate(tmp_num, calculStr[i * 2], tmp_oper)
        prev_oper = calculStr[i * 2 + 1]
